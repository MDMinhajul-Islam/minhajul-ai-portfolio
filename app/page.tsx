...
function ProjectDemo() {
  ...
  const labels = ['Image inpainting', 'AI workflow', 'Voice agent', 'Live chat', 'MRI classifier'];
  useEffect(() => { const timer = window.setInterval(() => setActive((value) => (value + 1) % labels.length), 4600); return () => window.clearInterval(timer); }, []);
  return <div className="demo-stage" aria-label={`Project demonstration: ${labels[active]}`}>
    <div className="demo-browser"><div className="demo-browser-bar"><i /><i /><i /><span>minhajul.lab / live-demo</span><b>RUNNING</b></div>
      <div className={`demo-scene inpaint-scene ${active === 0 ? 'active' : ''}`}><div className="before-pane"><span>INPUT</span><div className="photo-person" /></div><div className="process-arrow">→</div><div className="after-pane"><span>RECONSTRUCTED</span><div className="paint-sweep" /></div><div className="demo-result"><b>OBJECT REMOVED</b><small>background recovered</small></div></div>
      <div className={`demo-scene agent-scene ${active === 1 ? 'active' : ''}`}><div className="agent-query">Find a family SUV under $35k</div><div className="agent-flow"><span>VOICE / TEXT</span><i /><span>AI AGENT</span><i /><span>INVENTORY API</span></div><div className="vehicle-results"><div><b>2024 Toyota RAV4</b><small>$32,900 · Available</small></div><div><b>2023 Honda CR-V</b><small>$30,500 · Available</small></div></div></div>
      ...
      <div className={`demo-scene chat-scene ${active === 3 ? 'active' : ''}`}><div className="chat-head"><i>M</i><div><b>Project team</b><span>4 members online</span></div></div><div className="messages"><p className="incoming">The model evaluation is ready.</p><p className="outgoing">Great - sending the results now.</p><p className="incoming typing"><i/><i/><i/></p></div><div className="message-box">Write a message… <b>↗</b></div></div>
      <div className={`demo-scene mri-scene ${active === 4 ? 'active' : ''}`}><div className="scan-image"><div className="tumor-hotspot" /><span>MRI · AXIAL</span></div><div className="scan-data"><span>MODEL OUTPUT</span><h4>Glioma detected</h4><div><b>Confidence</b><strong>94.2%</strong></div><div className="confidence"><i /></div><small>Multiclass CNN · Figshare dataset</small></div></div>
    </div>
    <div className="demo-tabs">{labels.map((label, i) => <button key={label} className={active === i ? 'active' : ''} onClick={() => setActive(i)}><span>0{i + 1}</span>{label}<i /></button>)}</div>
  </div>;
}
...
export default function Home() {
  return <main>
    ...
    <section className="hero" id="top"><div className="hero-grid" aria-hidden="true" /><NeuralField /><div className="hero-copy"><p className="eyebrow"><span>01</span> AI ENGINEER · SOFTWARE BUILDER · RESEARCHER</p><h1>I build AI<br />that moves <em>beyond</em><br />the notebook.</h1><p className="hero-intro">I’m Md. Minhajul Islam - a CSE student and research intern who turns computer vision and deep-learning ideas into tested, usable software systems.</p><div className="hero-actions"><a className="primary-action" href="#work">See engineering evidence <ArrowDown size={17} /></a><a className="text-action" href={links.github} target="_blank" rel="noreferrer"><GitFork size={18} /> GitHub</a></div></div><div className="hero-orbit-label label-a">DIFFUSION<br /><span>GENERATIVE VISION</span></div><div className="hero-orbit-label label-b">APIs + AGENTS<br /><span>DEPLOYED SYSTEMS</span></div><div className="hero-side-note"><MousePointer2 size={14} /> MOVE TO BEND THE FIELD</div><div className="scroll-cue">SCROLL TO EXPLORE <span /></div></section>
    ...
    <section className="skills-section" id="skills"><div className="skills-intro"><p className="eyebrow"><span>02</span> ENGINEERING TOOLKIT</p><h2>Code, models,<br />and <em>systems.</em></h2><p>Working proficiency developed through coursework, research experiments, and complete software projects.</p></div><div className="skill-chart">{skills.map(([name,value],i)=><div className="skill-row" key={name} style={{'--value':`${value}%`,'--delay':`${i * .12}s`} as React.CSSProperties}><span>{name}</span><div><i /></div><b>{value}%</b></div>)}</div></section>
    ...
    <section className="research-section" id="research"><div className="research-evidence"><span className="evidence-kicker">EXPERIMENT PIPELINE</span><div className="pipeline-step"><b>01</b><span>Define the failure</span><small>Large masks, hallucinated texture, residual people</small></div><div className="pipeline-line"/><div className="pipeline-step"><b>02</b><span>Design the system</span><small>Segmentation, mask refinement, global + local inpainting</small></div><div className="pipeline-line"/><div className="pipeline-step"><b>03</b><span>Measure the result</span><small>Quality, runtime, parameter efficiency, residual detection</small></div><div className="research-code"><code>pipeline.run(image, mask)</code><strong>✓ reproducible output</strong></div></div><div className="research-copy"><p className="eyebrow"><span>05</span> RESEARCH PRACTICE</p><h2>Curiosity,<br />made <em>measurable.</em></h2><p>My current work investigates efficient image inpainting and object removal: designing lightweight architectures, building hybrid vision pipelines, and evaluating not just how outputs look, but what remains detectable.</p><div className="research-facts"><div><span>Current role</span><b>Research Intern</b><small>Image Segmentation & Object Removal</small></div><div><span>Thesis</span><b>Lightweight Diffusion Models</b><small>Efficient Image Inpainting</small></div><div><span>Institution</span><b>North South University</b><small>BSc, Computer Science & Engineering</small></div></div><div className="paper-list"><a href={links.scholar} target="_blank" rel="noreferrer"><span>01</span><p>A Machine Learning-Driven Framework for Enhancing Cognitive Function Using tDCS and Brain Gym Interventions</p><ArrowUpRight /></a><div><span>02</span><p>Machine Learning-Enhanced Cardiovascular Disease Risk Prediction: A Clinical Intelligence Framework</p><b>FORTHCOMING</b></div></div><a className="primary-action scholar" href={links.scholar} target="_blank" rel="noreferrer">Google Scholar <ArrowUpRight size={17} /></a></div></section>
