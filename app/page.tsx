...
function SkillMatrix() {
  ...
  useEffect(() => {
    ...
    let raf = 0;
    const tick = (now: number) => { const next = Math.min(1, (now - started) / 1450); setProgress(1 - Math.pow(1 - next, 3)); if (next < 1) raf = requestAnimationFrame(tick); };
    raf = requestAnimationFrame(tick);
    return () => cancelAnimationFrame(raf);
  }, [visible]);
  return <div ref={ref} className={`skill-chart ${visible ? 'is-live' : ''}`}>{skills.map(([name, value], i) => {
    const rowProgress = Math.max(0, Math.min(1, progress * 1.45 - i * .065));
...
function ProjectDemo() {
  ...
  const labels = ['Image inpainting', 'AI dealership', 'Voice agent', 'Live chat', 'MRI classifier', 'MindCare AI'];
  const carQueries = [
  ...
  return <div className="demo-stage" aria-label={`Project demonstration: ${labels[active]}`}>
    <div className="demo-browser"><div className="demo-browser-bar"><i /><i /><i /><span>minhajul.lab / live-demo</span><b>RUNNING</b></div>
      ...
      <div className={`demo-scene chat-scene ${active === 3 ? 'active' : ''}`}><div className="chat-head"><i>M</i><div><b>Engineering channel</b><span>4 members online · encrypted session</span></div></div><div className="messages"><p className="incoming">{chat[0]}</p><p className="outgoing">{chat[1]}</p><p className="incoming delivered">{chat[2]} <small>now</small></p><p className="incoming typing"><i/><i/><i/></p></div><div className="message-box">Write a message… <b>↗</b></div></div>
...
export default function Home() {
  return <main>
    ...
    <section className="skills-section" id="skills"><div className="skills-intro"><p className="eyebrow"><span>02</span> ENGINEERING TOOLKIT</p><h2>Code, models,<br />and <em>systems.</em></h2><p>Working proficiency developed through coursework, research experiments, and complete software projects. Scroll away and return to replay the live build.</p></div><SkillMatrix /></section>
